...
<? if($index == 0){ ?> <?= $line ?><? }else{ ?><?= mb_strtolower(mb_substr($line, 0, 1, 'utf-8'), 'utf-8') . mb_substr($line, 1, null, 'utf-8') ?><? } ?><? if($index < sizeof($ebook->CollectionMemberships) - 1){ ?>, <? } ?><? if($index == sizeof($ebook->CollectionMemberships) - 1){ ?>.<? } ?>
<? } ?>
