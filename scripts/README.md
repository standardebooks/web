...
The default web root is `/standardebooks.org/web/www`. If it is located elsewhere, specify it with the `--web-root` option. For instance, `deploy-ebook-to-www --web-root /var/www/html /path/to/ebook`. Note that there will be PHP errors if the Git repositories are not in the ebook directory immediately in the web root. Either keep them there or create a symlink.
