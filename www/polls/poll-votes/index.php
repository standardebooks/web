...
<main>
	<section class="narrow">
		...
		<? if($poll->IsActive()){ ?>
			<p class="center-notice">This poll closes on <?= $poll->End->format(Enums\DateTimeFormat::FullDateTime->value) ?> <?= SITE_TZ_STRING ?>.</p>
		<? }elseif($poll->End !== null){ ?>
			<p class="center-notice">This poll closed on <?= $poll->End->format(Enums\DateTimeFormat::FullDateTime->value) ?> <?= SITE_TZ_STRING ?>.</p>
		<? } ?>
		<table class="votes">
			<tbody>
