...
<p>Follow the general structure of the <code class="path">content.opf</code> file generated by <code class="bash"><span>se</span> create-draft</code>. Don’t rearrange the order of anything in there.</p>
...
<p>(Note: Strings can be made URL-safe using the <code class="bash"><span>se</span> make-url-safe</code> tool.)</p>
<ul>
...
<p>There are several elements in the metadata describing the publication date, updated date, and revision number of the ebook. Generally these are not updated by hand; instead, the <code class="bash"><span>se</span> prepare-release</code> tool updates them automatically.</p>
<ol type="1">
...
<li id="9.6.2.3"><aside class="number">9.6.2.3</aside><p>The long description is in <em>escaped</em> HTML, with the HTML beginning on its own line after the <code class="html"><span class="p">&lt;</span><span class="nt">meta</span> <span class="na">property</span><span class="o">=</span><span class="s">"se:long-description"</span><span class="p">&gt;</span></code> element.
						<aside class="tip">An easy way to escape HTML is to compose the long description in regular HTML, then insert it into the <code class="html"><span class="p">&lt;</span><span class="nt">meta</span> <span class="na">property</span><span class="o">=</span><span class="s">"se:long-description"</span><span class="p">&gt;</span></code> element surrounded by a <code class="html"><span class="cp">&lt;![CDATA[ ... ]]&gt;</span></code> element. Then, run the <code class="bash"><span>se</span> clean</code> tool, which will remove the <code class="html"><span class="cp">&lt;![CDATA[ ... ]]&gt;</span></code> element and escape the contained HTML.</aside>
</p></li>
...
<p>These two elements are automatically computed by the <code class="bash"><span>se</span> prepare-release</code> tool.</p>
<ol type="1">
...
<aside class="tip">The <code class="bash"><span>se</span> print-manifest-and-spine</code> tool generates a complete manifest that can be copied-and-pasted into the ebook’s metadata file.</aside>
<ol type="1">
...
<aside class="tip">The <code class="bash"><span>se</span> print-manifest-and-spine</code> tool generates a draft of the spine by making some educated guesses as to the reading order. The tool’s output is never 100% correct; manual review of the output is required, and adjustments may be necessary to correct the reading order.</aside>
</section>
</section>
		</article>
	</main>
