...
class HttpSessionInterface{
	 ...
	 *
	 * @return ($type is 'array' ? array<string>|null : ($type is 'bool' ? bool|null : ($type is 'float' ? float|null : ($type is 'int' ? int|null : ($type is 'string' ? string|null : ($type is 'empty-string' ? string|null : ($type is 'date'|'DateTimeImmutable' ? \Safe\DateTimeImmutable|null : ($type is class-string<T> ? T|null : mixed))))))))
	 */
